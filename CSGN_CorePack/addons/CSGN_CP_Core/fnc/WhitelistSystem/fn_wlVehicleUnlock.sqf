...
_isWhitelisted = [false, _caller, _vehicle] call CSGN_fnc_wlUidInWhitelist;

if (_isWhitelisted) then {
    _vehicle setVehicleLock "UNLOCKED";

    _infoText = format["This vehicle costs %1 Tickets on destruction.", _ticketprice];
    _infoText remoteExec ["hint", _caller, false]; 
}